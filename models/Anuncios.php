		...
		if(count($fotos) > 0) {
			for($q=0;$q<count($fotos['tmp_name']);$q++) {
				...
				if(in_array($tipo, array('image/jpeg', 'image/png'))) {
					...
					list($width_orig, $height_orig) = getimagesize(BASE_URL.'assets/images/anuncios/'.$tmpname);
					...
					if($tipo == 'image/jpeg') {
						$origi = imagecreatefromjpeg(BASE_URL.'assets/images/anuncios/'.$tmpname);
					} elseif($tipo == 'image/png') {
						$origi = imagecreatefrompng(BASE_URL.'assets/images/anuncios/'.$tmpname);
					}
